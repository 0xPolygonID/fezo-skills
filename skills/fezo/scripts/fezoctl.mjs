...
// node_modules/.pnpm/ajv@8.20.0/node_modules/ajv/dist/refs/json-schema-draft-06.json
var json_schema_draft_06_default = {
...
// src/engine/ajv-instance.ts
var OPTIONS = { allErrors: true, strict: false };
function normalizeDialect(uri) {
  return uri.replace(/^http:/, "https:").replace(/#$/, "");
}
var DIALECT_2020_12 = "https://json-schema.org/draft/2020-12/schema";
var DIALECT_2019_09 = "https://json-schema.org/draft/2019-09/schema";
var DIALECT_DRAFT_07 = "https://json-schema.org/draft-07/schema";
var DIALECT_DRAFT_06 = "https://json-schema.org/draft-06/schema";
function declaredDialect(schema) {
  if (typeof schema !== "object") return void 0;
  const declared = schema.$schema;
  return typeof declared === "string" ? normalizeDialect(declared) : void 0;
}
var SchemaCompiler = class {
  draft2020 = new import__2.Ajv2020(OPTIONS);
  byDialect;
  constructor() {
    const draft07 = new import_ajv.Ajv(OPTIONS);
    draft07.addMetaSchema(json_schema_draft_06_default);
    this.byDialect = /* @__PURE__ */ new Map([
      [DIALECT_2020_12, this.draft2020],
      [DIALECT_2019_09, new import__.Ajv2019(OPTIONS)],
   ...
   */
  compile(schema) {
    const ajv2 = this.instanceFor(schema);
    try {
      return ajv2.compile(schema);
    } catch (err) {
      ajv2.removeSchema(schema);
      throw err;
    }
  }
  instanceFor(schema) {
    const declared = declaredDialect(schema);
    if (declared === void 0) return this.draft2020;
    return this.byDialect.get(declared) ?? this.draft2020;
  }
};
function newSchemaCompiler() {
  return new SchemaCompiler();
}

// src/engine/tool-name.ts
import { createHash } from "node:crypto";
var MAX_TOOL_NAME_LENGTH = 64;
var TOOL_NAME_HASH_LENGTH = 4;
function methodToToolName(backendId, methodName) {
...
function findCandidateByToolName(candidates, toolName) {
  return candidates.find((candidate) => methodToToolName(candidate.backendId, candidate.method) === toolName);
}

// src/engine/catalog.ts
var CatalogFetchError = class extends Error {
  reason;
  status;
  body;
  constructor(reason, status, body) {
    super(
      reason === "status" ? `catalog fetch failed with status ${status}` : `catalog response with status ${status} could not be parsed as JSON`
    );
...
function formatBackendInfoText(value) {
  const rec = asRecord(value);
  if (!rec) return "";
  const parts = [];
  for (const key of ["title", "summary", "description"]) {
    const text = asString(rec[key]);
    if (text !== void 0 && text.length > 0) parts.push(text);
  }
  return parts.join(" \u2014 ");
}
function parseBillingModel(value) {
  const rec = asRecord(value);
  const model = rec ? asString(rec.model) : void 0;
...
function parseHttpBody(value) {
  ...
  if (contentRec) {
    ...
    for (const [mediaType, mediaValue] of Object.entries(contentRec)) {
      ...
      if (schema !== void 0) media.schema = schema;
...
function parseHttpBindings(value) {
  const rec = asRecord(value);
  if (!rec) return {};
  const bindings = {};
  const method = asString(rec.method);
  if (method !== void 0) bindings.method = method;
  const query = asStringArray(rec.query);
  if (query !== void 0) bindings.query = query;
  const pathParams = asStringArray(rec.path_params);
  if (pathParams !== void 0) bindings.path_params = pathParams;
  const header = asStringArray(rec.header);
  if (header !== void 0) bindings.header = header;
  const requestBody = parseHttpBody(rec.request_body);
  if (requestBody !== void 0) bindings.request_body = requestBody;
  const responseBody = parseHttpBody(rec.response_body);
  if (responseBody !== void 0) bindings.response_body = responseBody;
  return bindings;
}
...
function resolveHttpMethod(bindings, backendId, methodName) {
  const declared = bindings.method;
  if (declared === void 0 || declared.length === 0) return "POST";
...
function resolvePath(path, methodName) {
  const raw = path && path.length > 0 ? path : methodName;
  return raw.startsWith("/") ? raw : `/${raw}`;
}
function normalizeCatalog(parsed) {
  const root = asRecord(parsed);
  const backendsRaw = root ? asArray(root.backends) : void 0;
...
function formatBindingError(reason, names) {
...
function isMissingBodyValue(value) {
  return value === void 0;
}
function requiredPropertyNames(schema) {
...
function bindArgs(candidate, args, bodyJson) {
  ...
  if (bodyJson !== void 0 && candidate.httpMethod === "GET") {
    throw new BindingError("body-not-allowed", []);
  }
  const pathParamNames = /* @__PURE__ */ new Set();
  const missingPath = [];
  const resolvedPath = candidate.path.replace(/\{(\w+)\}/g, (_match, name) => {
  ...
  if (bodyJson !== void 0) {
    ...
    const missingBody = requiredNames.filter(
      (name) => !pathParamNames.has(name) && !queryNames.includes(name) && !headerNames.includes(name) && isMissingBodyValue(source[name])
    );
    ...
    if (unbound.length > 0) {
      warn2(
        `${candidate.tool}: not sending argument(s) ${unbound.join(", ")} -- nothing placed them in the path, query string, or a header, and a GET request has no body`
      );
    }
  }
  return {
...
async function readSecretFromStream(stream) {
  const chunks = [];
...
var KEYCHAIN_ACCOUNT = "fezo";
var KEYCHAIN_SERVICE_URL = "fezo-url";
var KEYCHAIN_SERVICE_API_KEY = "fezo-api-key";
function writeKeychainSecret(runner, service, account, secret) {
...
function readKeychainSecret(runner, service, account) {
...
function readDotEnvFile(path) {
  ...
  try {
    ...
    if (nodeErrorCode(err) !== "ENOENT") {
      ...
      warn3(`could not read ${path}: ${message}`);
    }
    return {};
  }
  return parseDotEnv(text);
}
function writeDotEnvFile(path, values) {
  try {
...
function storeCredentials(options) {
  ...
  if (!options.keychain) {
    const outcome = { ok: false, reason: "no-keychain-runner", message: "keychain storage requires a KeychainRunner" };
    return {
      storage: "keychain",
      apiKey: outcome,
      ...url !== void 0 ? { url: outcome } : {}
...
function inferCapability(intent) {
  ...
  if (allMatches.length === 1) {
    ...
    if (only === void 0) return { kind: "none" };
  ...
  if (phraseWinners.length === 1) {
    ...
    if (winner === void 0) return { kind: "ambiguous", candidates: allMatches };
...
function tokenize(query) {
  return query.toLowerCase().split(/[^a-z0-9]+/).filter((token) => token.length > 0);
}
function removeStopWords(tokens) {
  const filtered = tokens.filter((token) => !STOP_WORDS.has(token));
  return filtered.length > 0 ? filtered : [...tokens];
}
function searchableBlob(candidate) {
...
var IDENTIFIER_TERM_WEIGHT = 3;
var TITLE_TERM_WEIGHT = 2;
var REST_TERM_WEIGHT = 1;
function fieldWeight(term, blob) {
...
function searchCandidates(candidates, query) {
  ...
  for (const candidate of candidates) {
    ...
    if (allTermsMatch || exactMatch !== void 0) {
      matches.push({
        candidate,
        matchedTerms,
        ...exactMatch !== void 0 ? { exactMatch } : {}
      });
    }
  }
  return matches;
}
var ASYNC_NAME_SUFFIXES = ["_async", "_status", "_progress", "_snapshot", "_get", "_dataset"];
var ASYNC_TEXT_PHRASES = [
  "asynchronous",
  "async",
  "job id",
  "snapshot_id",
...
var ASYNC_ID_PROPERTY_NAMES = /* @__PURE__ */ new Set(["id", "job_id", "snapshot_id", "request_id", "task_id"]);
function hasAsyncNamePattern(candidate) {
...
function hasAsyncTextPhrase(candidate) {
...
function isAsyncLifecycleMethod(candidate) {
  return hasAsyncNamePattern(candidate) || hasAsyncTextPhrase(candidate) || hasAsyncOutputShape(candidate);
}
var ASYNC_OVERRIDE_TERMS = /* @__PURE__ */ new Set(["async", "job", "snapshot", "status", "crawl"]);
function queryRequestsAsyncBehavior(intent) {
...
function classifyTier(match, rawTokens) {
  if (match.exactMatch === "tool") return "exact-tool";
  const hasBackendToken = match.exactMatch === "backend" || rawTokens.has(match.candidate.backendId.toLowerCase());
...
function billingWeight(model) {
  if (model === "package") return 0;
  if (model === "per_call") return 1;
  return 2;
}
function rankCandidates(matches, intent, capability) {
  const rawTokens = new Set(tokenize(intent));
  const preferenceList = capability !== void 0 ? CAPABILITY_PREFERENCES[capability] : void 0;
  const withRank = matches.map((match, originalIndex) => {
    ...
    const explanation = {
      ...
      ...capability !== void 0 && preferencePosition >= 0 ? { preference: { capability, position: preferencePosition } } : {}
    };
...
function selectForRun(candidates, intent) {
...
function parseResultBody(bodyText) {
  try {
...
function summarizeCandidateLine(candidate) {
  const title = candidate.title !== void 0 ? ` \u2014 ${candidate.title}` : "";
...
function explainRank(explanation) {
  const parts = [explanation.tier];
  ...
  if (explanation.preference !== void 0) {
    parts.push(`preferred for "${explanation.preference.capability}" (position ${String(explanation.preference.position)})`);
  }
  return parts.join("; ");
}
function schemaAndBindings(candidate) {
  return {
    httpMethod: candidate.httpMethod,
    path: candidate.path,
    bindings: candidate.bindings,
    inputSchema: candidate.inputSchema,
    ...candidate.outputSchema !== void 0 ? { outputSchema: candidate.outputSchema } : {}
  };
}
function renderBindingsText(bindings) {
  const lines = [];
  if (bindings.method !== void 0) lines.push(`  method: ${bindings.method}`);
  if (bindings.query !== void 0) lines.push(`  query: [${bindings.query.join(", ")}]`);
  if (bindings.path_params !== void 0) lines.push(`  path_params: [${bindings.path_params.join(", ")}]`);
  if (bindings.header !== void 0) lines.push(`  header: [${bindings.header.join(", ")}]`);
  if (bindings.request_body !== void 0) lines.push(`  request_body: ${toJson(bindings.request_body)}`);
  if (bindings.response_body !== void 0) lines.push(`  response_body: ${toJson(bindings.response_body)}`);
  return lines.length > 0 ? lines.join("\n") : "  (no declared binding \u2014 defaults apply)";
}
function renderVersion(version, json) {
...
function renderSearch(ranked, query, options) {
  if (options.json) {
    return toJson({
      query,
      count: ranked.length,
  ...
  ranked.forEach(({ candidate, explanation }, index) => {
    ...
    if (options.includeSchema) {
      ...
      if (candidate.outputSchema !== void 0) lines.push(`   output_schema: ${toJson(candidate.outputSchema)}`);
      lines.push(renderBindingsText(candidate.bindings));
    }
  });
  return lines.join("\n");
}
function renderSchema(candidate, json) {
  if (json) {
...
var BILLING_STATEMENT = "every attempt that reached a 2xx response is billed by the provider (billed=true); attempts that failed or were skipped before a request was sent are not billed";
function renderAttemptsText(attempts) {
  if (attempts.length === 0) return ["attempts: (none)"];
  const lines = ["attempts:"];
  attempts.forEach((attempt, index) => {
    const httpStatus = attempt.httpStatus !== void 0 ? ` httpStatus=${String(attempt.httpStatus)}` : "";
    const gatewayCode = attempt.gatewayCode !== void 0 ? ` gatewayCode=${attempt.gatewayCode}` : "";
    lines.push(
...
function renderCall(input, json) {
  const billedAnyAttempt = input.report.attempts.some((attempt) => attempt.billed);
  if (json) {
    return toJson({
      tool: input.tool,
      resolved: input.candidate !== void 0,
      ...input.candidate !== void 0 ? { backendId: input.candidate.backendId, method: input.candidate.method } : {},
      ...input.boundRequest !== void 0 ? { request: input.boundRequest } : {},
...
function renderRun(input, json) {
  ...
  if (json) {
    ...
    switch (selection.outcome) {
      ...
      default: {
        const exhaustive = selection;
        return exhaustive;
      }
    }
    if (input.report !== void 0) {
  ...
  switch (selection.outcome) {
    ...
    case "refused-unhinted-multi-backend":
      lines.push(
        `refused: candidates span multiple backends with no capability preference (${selection.reason.backends.join(", ")}); use --allow-unhinted-auto-pick to pick the top-ranked one, or call a specific tool`
      );
      if (input.allowUnhintedAutoPick) {
        const promoted = input.runCandidates?.[0];
        if (promoted !== void 0) lines.push(`--allow-unhinted-auto-pick set: promoting ${summarizeCandidateLine(promoted)}`);
      }
      break;
    default: {
      const exhaustive = selection;
...
function renderDoctor(checks, json) {
  ...
  for (const check of checks) {
    ...
    if (check.details !== void 0) lines.push(`      ${toJson(check.details)}`);
  }
  return lines.join("\n");
}
...
function candidatesToRun(selection, allowUnhintedAutoPick) {
  switch (selection.outcome) {
    ...
    case "refused-unhinted-multi-backend": {
      ...
      return top !== void 0 ? [top.candidate] : [];
    }
    case "selected":
...
var NO_MORE_CANDIDATES_REASON = "no more candidates to try";
function unresolvedToolReport(tool) {
  const failure = {
    kind: "gateway",
    status: 404,
  ...
  const attempt = {
    ...
    ...classified.gatewayCode !== void 0 ? { gatewayCode: classified.gatewayCode } : {}
  };
  return { attempts: [attempt], outcome: { kind: "give_up", reason: NO_MORE_CANDIDATES_REASON } };
}
var probeCompiler = newSchemaCompiler();
...
function validateBodyAgainstBinding(cache, candidate, bodyJson) {
  const schema = candidate.bindings.request_body?.content?.["application/json"]?.schema;
  if (schema === void 0) return { valid: true };
...
function emitUsageError(emit, command, message) {
...
function credentialResolutionFor(deps) {
  return resolveCredentials({
    env: deps.env ?? process.env,
    ...deps.dotEnvPath !== void 0 ? { dotEnvPath: deps.dotEnvPath } : {},
    ...deps.keychain !== void 0 ? { keychain: deps.keychain } : {}
  });
}
...
async function openGateway(deps, emit) {
...
function parseJsonFlag(raw, flagName, command, emit) {
  try {
...
function verifyStoredField(outcome, expectedSource, resolved2, expectedValue) {
  ...
  if (resolved2 !== void 0 && resolved2.source !== expectedSource) {
    return {
      ok: true,
      reason: `unverified-shadowed-by-${resolved2.source}`,
...
async function cmdSetup(flags, deps, emit) {
  ...
  const stored = storeCredentials({
    storage,
    apiKey,
    ...flags.url !== void 0 ? { url: flags.url } : {},
    ...deps.dotEnvPath !== void 0 ? { dotEnvPath: deps.dotEnvPath } : {},
    ...deps.keychain !== void 0 ? { keychain: deps.keychain } : {}
  });
  const resolution = credentialResolutionFor(deps);
...
async function runCli(argv, deps = {}) {
  ...
  const finish = (exitCode) => ({ exitCode, stdout: outLines.join(""), stderr: errLines.join("") });
  const preParseEmit = { out: write, err: writeErr, json: argv.includes("--json") };
  const first = argv[0];
  const wantsHelp = argv.some((token) => token === "--help" || token === "-h");
  if (first === void 0 || wantsHelp) {
    write(HELP_TEXT);
