...
// problems, backend_not_configured, ...) are always written by a single
// error-writing helper as a top-level
// `{ "error": { "code": "...", "message": "..." } }` envelope. A backend's own
// non-2xx response is proxied through byte-for-byte instead of being
// re-wrapped: a backend that calls an upstream provider forwards that
// provider's status and body verbatim, and the body can be shaped however the
// provider's own API shapes errors (FastAPI's `{"detail": "..."}`, plain text,
// or anything else). Treating every non-2xx body as one of these two shapes,
// and never inventing a `code` for the second, is the gateway's documented
// backend-authoring contract -- and the one the old MCP client's
// `parseErrorBody` violated by falling back through
// `detail`/`message`/`JSON.stringify` guesses.
//
...
//    (HTTP 429) -- not on any `/v1/*` tool call. fezoctl never hits that
//    endpoint, so a `{kind:'gateway', code:'rate_limited'}` envelope is not
//    normally observable in practice.
...
// 2. `limit_exceeded` (HTTP 402, the gateway's spend-limit check) can be
//    scoped to the whole account, one API key, or one backend, but the gateway
//    exposes that scope only in the human-readable message string -- never as
//    a structured field on the wire. retry.ts does not parse that message, so
//    it aborts the whole run conservatively even when the limit is
//    backend-scoped and a different candidate could, in principle, have safely
//    advanced instead.
// ---------------------------------------------------------------------------
