...
var PROCESS_ALIAS_WARNINGS = /* @__PURE__ */ new Set();
...
function resolveOne(options) {
  ...
  if (deprecated !== void 0 && deprecated.length > 0) {
    if (!options.warnedAliases.has(options.deprecatedName)) {
      options.warnedAliases.add(options.deprecatedName);
      warn3(`${options.deprecatedName} is deprecated; use ${options.canonicalName} instead`);
    }
...
var probeAjv = new import_ajv2.Ajv({ allErrors: true, strict: false });
...
var HELP_TEXT = `fezoctl \u2014 discover and call Fezo/Zug gateway tools from the live catalog
